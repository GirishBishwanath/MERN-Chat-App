...
type FindSession = (sessionId: string, userId: string) => Promise<{ id: string } | null>;
...
const secureRoute = async (
  ...
  { findUser = findPublicById, findSession = findActiveSessionById }: SecureRouteOptions = {}
): Promise<void> => {
  try {
    ...
    if (!session) throw new AppError("Invalid session", 401, ERROR_CODES.UNAUTHENTICATED);
    const user = await findUser(decoded.userId);
    if (!user) throw new AppError("Invalid session", 401, ERROR_CODES.UNAUTHENTICATED);
    req.user = user;
    next();
  } catch (error) {
    next(error);
