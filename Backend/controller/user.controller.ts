...
import {
  clearAuthCookies, refreshSession, revokeAllSessions, revokeRefreshSession, setAuthCookies,
} from "../auth/session.js";
import { AppError } from "../errors/AppError.js";
import { ERROR_CODES } from "../errors/errorCodes.js";
import type { AuthenticatedRequest } from "../types/http.js";
import {
  authenticateUser, getPublicUser, getUsersExcept, registerUser, sanitizeUser,
...
export const login = async (req: Request, res: Response): Promise<Response> => {
  ...
  await setAuthCookies(user._id, res);
  return res.status(200).json({ message: "User logged in successfully", user: sanitizeUser(user) });
};
...
export const me = (req: AuthenticatedRequest, res: Response): Response =>
...
export const allUsers = async (req: AuthenticatedRequest, res: Response): Promise<Response> =>
  res.status(200).json(await getUsersExcept(req.user._id));
