...
const mapSession = (row: Record<string, unknown>): PostgresSession => ({
  ...
  expiresAt: new Date(String(row.expires_at)), lastUsedAt: new Date(String(row.last_used_at)),
  createdAt: new Date(String(row.created_at)), updatedAt: new Date(String(row.updated_at)),
});

export const createSession = async (input: {
  userId: string; tokenHash: string; expiresAt: Date; lastUsedAt: Date;
...
export const findActiveSessionByTokenHash = async (tokenHash: string, now = new Date()): Promise<PostgresSession | null> => {
  const result = await postgresPool.query(`
    UPDATE sessions SET last_used_at = $2, updated_at = $2
...
export const findActiveSessionById = async (
  ...
  const result = await postgresPool.query(`
    SELECT id, user_id, token_hash, expires_at, last_used_at, created_at, updated_at
    FROM sessions
